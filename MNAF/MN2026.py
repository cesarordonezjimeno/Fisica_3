...
def raiznsima(a, n, niter, tol):
    ...
    for i in range (niter):    # Bucle para calcular 
        xk1 = xk * c1 + c2 / xk ** (n - 1)
        if abs(xk1 - xk) < tol:     # Condición para detener bucle
            break
        xk = xk1        # Sustituir el valor y continuar calculando
    return xk1
...
def euclides(D, d):
    """
Calcula el máximo común divisor de dos enteros con el algoritmo de Euclides.
    Parámetros de entrada:
        D  (int)    : valor entero
        d  (int)    : valor entero 
    Salida:
        xkm1  : aproximación de la raíz n-ésima de a
    """
    c, r = divmod (D, d)        # Cocientes y resto
    while r != 0:               # Repetir mientras no sea distinto de cero
        D, d = d, r             # Sustituir valores
        c, r = divmod (D, d)
        return d                # EL mcd
